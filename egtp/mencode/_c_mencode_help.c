 ...
 * 
 * $Id: _c_mencode_help.c,v 1.3 2003/03/05 05:51:16 myers_carpenter Exp $
 */
...
static PyObject *colon_tup_const, *close_paren_tup_const, *str_tup_const,
  *dict_tup_const;
...
static PyObject *
real_encode_string (PyObject * data, PyObject * output)
{
  ...
  if (!owrite_method || !PyCallable_Check (owrite_method))
    {
      excval =
	Py_BuildValue ("(is)", 0,
		       "output object must have a callable write method");
      if (excval != NULL)
	{
...
static PyObject *
encode_string (PyObject * self, PyObject * args)
{
  ...
  if (!PyArg_ParseTuple (args, "OO:encode_string", &data, &output))
    {
      excval =
	Py_BuildValue ("(is)", 0,
		       "expected two python objects as parameters");
      if (excval != NULL)
	{
	  PyErr_SetObject (PyExc_ValueError, excval);
	  Py_DECREF (excval);
...
static PyObject *
real_encode_dict (PyObject * data, PyObject * output)
{
  ...
  if (PyList_Sort (keys) != 0)
    {
      excval =
	Py_BuildValue ("(is)", 0, "PyList_Sort failed [returned non zero]");
      if (excval != NULL)
	{
  ...
  for (i = 0; i < keylen; i++)
      ...
      if (!PyString_Check (key) && !PyInt_Check (key) && !PyLong_Check (key))
	{
	  ...
	  excval =
	    Py_BuildValue ("(is)", 0,
			   "mencoded dictionary keys must be strings or numbers");
	  if (excval != NULL)
	    {
	      PyErr_SetObject (MencodeError, excval);
	      Py_DECREF (excval);
...
static PyObject *
encode_dict (PyObject * self, PyObject * args)
{
...
static PyObject *
real_encode_io (PyObject * data, PyObject * output)
{
  ...
  if (PyString_Check (data))
      ...
      if (func == NULL)
	{
	  ...
	  excval =
	    Py_BuildValue ("(is)", 0,
			   "encoder_dict did not contain an encoding function for data");
	  if (excval != NULL)
	    {
	      PyErr_SetObject (MencodeError, excval);
	      Py_DECREF (excval);
...
static PyObject *
encode_io (PyObject * self, PyObject * args)
{
  PyObject *data, *output;
  PyObject *excval;
...
static PyObject *
decode_raw_string (PyObject * self, PyObject * args)
{
  ...
  if (!PyArg_ParseTuple (args, "Oi:decode_raw_string", &dataobj, &index))
    {
      excval =
	Py_BuildValue ("(is)", 0,
		       "expected a string and an integer as parameters");
      if (excval != NULL)
	{
	  PyErr_SetObject (PyExc_ValueError, excval);
	  Py_DECREF (excval);
  ...
  if (index < 0)
    {
      excval =
	Py_BuildValue ("(is)", 0, "index must be a non-negative integer");
      if (excval != NULL)
	{
	  PyErr_SetObject (PyExc_ValueError, excval);
	  Py_DECREF (excval);
...
/*PyString_AsStringAndSize(dataobj, &dataptr, &dataptrlen); *//* python 1.5.2 doesn't have this function */
      ...
      if (!dataptr)
	{
	  excval =
	    Py_BuildValue ("(is)", 0,
			   "first parameter was not a python object?");
	  if (excval != NULL)
	    {
      ...
      if (index > dataptrlen)
	{
	  excval =
	    Py_BuildValue ("(is)", 0,
			   "index is greater than the string length");
	  if (excval != NULL)
	    {
	      PyErr_SetObject (PyExc_ValueError, excval);
	      Py_DECREF (excval);
  ...
  else if (PyBuffer_Check (dataobj))
    {
      ...
      /* TODO accept buffer objects */
      excval =
	Py_BuildValue ("(is)", 0, "Buffer objects are not yet supported");
      if (excval != NULL)
	{
	  PyErr_SetObject (PyExc_TypeError, excval);
	  Py_DECREF (excval);
    ...
    {
      excval =
	Py_BuildValue ("(is)", 0, "parameter must be a python String object");
      if (excval != NULL)
	{
	  PyErr_SetObject (PyExc_TypeError, excval);
	  Py_DECREF (excval);
  ...
  while (digitptr < colonptr)
	...
	{
	  excval =
	    Py_BuildValue ("(is)", 0, "length contained non-digit character");
	  if (excval != NULL)
	    {
	      PyErr_SetObject (MencodeError, excval);
	      Py_DECREF (excval);
  ...
  if ((strlength != 0) && (*dataptr == '0'))
    {
      excval =
	Py_BuildValue ("(is)", 0,
		       "positive string length must not begin with `0'");
      if (excval != NULL)
	{
	  PyErr_SetObject (MencodeError, excval);
	  Py_DECREF (excval);
...
init_c_mencode_help (void)
{
  ...
  MencodeError =
    PyErr_NewException ("mencode._c_MencodeError", PyExc_StandardError, NULL);
